    ...
    <div class="flex items-center gap-6 flex-1">
        ...
        <div class="relative w-full max-w-md hidden md:block group">
            <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                <svg class="h-5 w-5 text-slate-400 group-focus-within:text-wa-teal transition-colors duration-300"
            ...
            <input type="text"
                class="block w-full pl-11 pr-4 py-3 bg-slate-100/50 dark:bg-slate-900/50 border-none rounded-2xl text-sm font-bold text-slate-700 dark:text-slate-200 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-wa-teal/50 focus:bg-white dark:focus:bg-slate-900 transition-all duration-300"
                placeholder="Search contacts, messages, or settings... (Cmd+K)" @focus="isSearchFocused = true"
    ...
    <div class="flex items-center gap-4">
        ...
        <div class="relative" x-data="{ open: false }">
            <button @click="open = !open"
                class="relative p-2.5 rounded-xl text-slate-400 hover:text-wa-teal hover:bg-slate-100 dark:hover:bg-slate-800 transition-all duration-300 focus:outline-none">
                <span
                ...
                <x-dropdown align="right" width="60">
                    <x-slot name="trigger">
                        <button type="button"
                            ...
                            <svg class="h-4 w-4 text-slate-400 group-hover:text-wa-teal transition-colors"
                    ...
                    <x-slot name="content">
                        <div class="w-60">
                            <div class="block px-4 py-2 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                                {{ __('Manage Team') }}
                            </div>
            ...
            <x-dropdown align="right" width="48">
                <x-slot name="trigger">
                    @if (Laravel\Jetstream\Jetstream::managesProfilePhotos())
                        <button
                            class="flex text-sm border-2 border-transparent rounded-full focus:outline-none focus:border-wa-teal transition shadow-lg shadow-black/10">
