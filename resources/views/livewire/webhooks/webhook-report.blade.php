<div class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <!-- Modal Panel -->
        <div
            class="inline-block align-bottom bg-white dark:bg-slate-900 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-6xl sm:w-full">
            ...
            <div
                ...
                <div class="mt-2 text-sm text-slate-500">
                    <span class="font-bold text-slate-900 dark:text-white">{{ $this->workflow->name }}</span> (Anytime)
                </div>
            </div>

            <div class="bg-gray-50 dark:bg-slate-950 p-6 space-y-6">
                <!-- Info Cards -->
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div
                        ...
                        <div>
                            <div class="text-xs font-bold uppercase text-blue-500">Status</div>
                            <div class="text-lg font-black text-slate-900 dark:text-white">Completed</div>
                        </div>
                    </div>
                    <div
                        class="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 flex items-center gap-4">
                        ...
                        <div>
                            <div class="text-xs font-bold uppercase text-cyan-500">Targeted</div>
                    ...
                    <div
                        ...
                        <div>
                            <div class="text-xs font-bold uppercase text-teal-500">Message Count</div>
                            <div class="text-lg font-black text-slate-900 dark:text-white">{{ $stats['total'] }}</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div
                        ...
                        <div class="flex-1">
                            ...
                            <div class="text-[10px] text-slate-400 mb-1">Sent (100%)</div>
                    ...
                    <div
                        ...
                        <div class="flex-1">
                            ...
                            <div class="text-[10px] text-slate-400 mb-1">Opened
                                ({{ $stats['total'] > 0 ? round(($stats['read'] / $stats['total']) * 100) : 0 }}%)</div>
                            <div class="h-1 bg-slate-100 rounded-full w-full overflow-hidden">
                    ...
                    <div
                        ...
                        <div class="flex-1">
                            <div class="flex justify-between text-xs font-bold mb-1">
                                <span class="text-gray-500">Unreached</span>
                ...
                <div
                    ...
                    <div class="px-8 py-4 border-b border-slate-50 dark:border-slate-800 flex items-center gap-4">
                        <x-input wire:model.live.debounce.300ms="search" placeholder="Search & Enter..."
                            class="w-64 bg-slate-50 border-none rounded-xl" />
                    </div>
                            ...
                            <tbody class="divide-y divide-slate-50 dark:divide-slate-800/30">
                                @forelse($messages as $msg)
                                    <tr class="group hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors">
                                        ...
                                        <td class="px-8 py-4 text-sm font-bold text-blue-500">{{ $msg->chat_id }}</td>
                                        <td class="px-8 py-4">
                                            <span
                                                class="text-xs font-bold {{ $msg->status == 'read' ? 'text-green-500' : ($msg->status == 'delivered' ? 'text-green-500' : 'text-slate-500') }}">
                                                {{ ucfirst($msg->status) }}
                                            </span>
                                        </td>
                                        <td class="px-8 py-4 text-xs font-medium text-slate-500">
                                            <div>{{ $msg->sent_at?->format('d M') }}</div>
                                            <div>{{ $msg->sent_at?->format('H:i') }}</div>
                                        </td>
                                        <td class="px-8 py-4 text-xs font-medium text-slate-500">
                                            @if($msg->delivered_at)
                                        ...
                                        <td class="px-8 py-4 text-xs font-medium text-slate-500">
                                            ...
                                            @else
                                                <div class="text-center text-slate-300"><svg class="w-4 h-4 mx-auto" fill="none"
                                                        stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                            d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                                    </svg></div>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="7" class="px-8 py-10 text-center text-slate-500">No logs found.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
