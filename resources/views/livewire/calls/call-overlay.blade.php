<div x-data="{ 
    ...
    <div x-show="isVisible" x-transition:enter="transition cubic-bezier(0.34, 1.56, 0.64, 1) duration-500"
        ...
        x-transition:leave-end="opacity-0 scale-90 blur-xl translate-y-[-20%]" x-cloak
        ...
        <div class="relative flex items-center gap-4 px-3 py-2.5">
            ...
            <div class="relative flex-shrink-0">
                ...
                <img src="{{ $contactAvatar }}"
                    class="relative w-12 h-12 rounded-full border-2 border-white/10 shadow-inner object-cover" alt="">
                ...
                <div class="flex items-center gap-1.5 mt-0.5">
                    ...
                    <template x-if="status === 'active'">
                        <span class="flex items-center gap-1">
                            ...
                            <span class="text-[10px] font-mono text-wa-teal font-black"
            ...
            <div class="flex items-center gap-2 pr-2">
                ...
                <template x-if="status === 'ringing' && direction === 'inbound'">
                    <div class="flex items-center gap-2">
                        <button @click="performAction('rejectCall')"
                            class="w-10 h-10 rounded-full bg-rose-500 text-white flex items-center justify-center hover:bg-rose-600 hover:scale-110 active:scale-90 transition-all shadow-lg">
                        ...
                        <button @click="performAction('answerCall')"
                            class="w-10 h-10 rounded-full bg-wa-teal text-white flex items-center justify-center hover:bg-wa-teal-dark hover:scale-110 active:scale-95 transition-all shadow-lg animate-bounce">
                ...
                <!-- End Call / Cleanup -->
                <template
                    x-if="(status === 'active' || direction === 'outbound' || status === 'ended') && status !== 'idle'">
                    <div class="flex items-center gap-2">
                            ...
                            <div class="flex items-end gap-0.5 h-3 mr-2">
                                <template x-for="i in 4">
                                    <div class="w-1 rounded-full transition-all duration-500"
                                        :class="signalQuality >= (i*25) ? 'bg-wa-teal' : 'bg-white/10'"
                        ...
                        <button @click="status === 'ended' ? resetLocal() : performAction('endCall')"
                            ...
                            <svg x-show="!isProcessing" class="w-5 h-5"
                                :class="status === 'active' ? 'rotate-[135deg]' : ''" fill="currentColor"
                                viewBox="0 0 24 24">
                                <path x-show="status !== 'ended'"
                                    d="M21 15.46l-5.27-.61-2.52 2.52c-2.83-1.44-5.15-3.75-6.59-6.59l2.53-2.53L8.54 3H3.01C2.45 13.18 10.82 21.55 21 20.99v-5.53z" />
                                <path x-show="status === 'ended'"
                                    d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z" />
                            </svg>
                            <svg x-show="isProcessing" class="animate-spin h-5 w-5" viewBox="0 0 24 24">
        ...
        <button @click="showDebug = !showDebug"
            class="absolute top-1 right-12 w-4 h-4 rounded-full bg-white/5 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
        ...
        <div x-show="showDebug" x-collapse
            ...
            <template x-for="logEntry in debugLogs" :key="logEntry.id">
                <div class="mb-1">
                    <span class="text-white/20" x-text="logEntry.time"></span>
                    <span
                        :class="{ 'text-wa-teal': logEntry.type === 'success', 'text-rose-400': logEntry.type === 'error', 'text-amber-300': logEntry.type === 'warn' }"
                        x-text="logEntry.msg"></span>
                </div>
            </template>
        </div>
    </div>
