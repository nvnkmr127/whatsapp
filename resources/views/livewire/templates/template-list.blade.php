                ...
                <div class="p-8 md:p-10 overflow-y-auto custom-scrollbar h-full border-r border-slate-50 dark:border-slate-900 bg-white dark:bg-slate-950">
                    <div class="space-y-10">
                        ...
                        <section class="space-y-6">
                            ...
                            <div x-data="{ 
                                insertText(type, token = '') {
                                    let el = $refs.bodyTextarea;
                                    ...
                                    let newValue = text.substring(0, start) + replacement + text.substring(end);
                                    $wire.set('body', newValue);
                                    
                                    setTimeout(() => {
                                        el.focus();
                                        let newCursor = start + replacement.length;
                                ...
                                <div class="flex items-center justify-between mb-2">
                                    <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest">Message Body (Required)</label>
                                    <div class="flex items-center gap-1.5 {{ $category === 'AUTHENTICATION' ? 'hidden' : '' }}">
                                        <button type="button" @click="insertText('format', '*')" class="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg text-slate-500 hover:text-wa-teal transition-colors" title="Bold">
                                            <span class="font-bold text-sm">B</span>
                                        </button>
                                        <button type="button" @click="insertText('format', '_')" class="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg text-slate-500 hover:text-wa-teal transition-colors" title="Italic">
                                            <span class="italic text-sm">I</span>
                                        </button>
                                        <button type="button" @click="insertText('format', '~')" class="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg text-slate-500 hover:text-wa-teal transition-colors" title="Strikethrough">
                                            <span class="line-through text-sm">S</span>
                                        </button>
                                        <div class="w-px h-4 bg-slate-200 dark:bg-slate-800 mx-1"></div>
                                        <div class="relative">
                                            <button type="button" @click="showEmojiPicker = !showEmojiPicker" class="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg text-slate-500 hover:text-wa-teal transition-colors" title="Emojis">
                                            ...
                                            <div x-show="showEmojiPicker" @click.away="showEmojiPicker = false" x-cloak
                                                class="absolute bottom-full right-0 mb-2 p-3 bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-100 dark:border-slate-800 grid grid-cols-6 gap-2 z-50 animate-in fade-in slide-in-from-bottom-2 duration-200 w-48">
                                                @foreach(['😀', '😃', '😄', '😁', '😅', '😂', '🤣', '😊', '😇', '🙂', '🙃', '😉', '😌', '😍', '🥰', '😘', '😗', '😙', '😚', '😋', '😛', '😝', '😜', '🤪', '🤨', '🧐', '🤓', '😎', '🤩', '🥳'] as $emoji)
                                                    <button type="button" @click="insertText('emoji', '{{ $emoji }}'); showEmojiPicker = false" class="text-lg hover:scale-125 transition-transform">{{ $emoji }}</button>
                                                @endforeach
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <textarea wire:model.live="body" x-ref="bodyTextarea" rows="5" {{ $category === 'AUTHENTICATION' ? 'readonly' : '' }}
                                ...
                                @foreach($buttons as $index => $btn)
                                    <div class="p-6 bg-slate-50/50 dark:bg-slate-900/50 rounded-3xl border border-slate-100 dark:border-slate-800 relative group transition-all">
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4" x-data="{
                                            insertToButton(field, type, token = '') {
                                                let el = $refs[field + 'Input_' + {{ $index }}];
                                                let start = el.selectionStart;
                                                let end = el.selectionEnd;
                                                let text = el.value;
                                                let replacement = '';
                                                ...
                                                let newValue = text.substring(0, start) + replacement + text.substring(end);
                                                $wire.set('buttons.' + {{ $index }} + '.' + field, newValue);
                                            ...
                                            <div>
                                                <label class="block text-[9px] font-black text-slate-400 uppercase mb-3 flex items-center justify-between">
                                                    <span>Label (Max 25)</span>
                                                    <div class="flex items-center gap-1.5">
                                                        <button type="button" @click="insertToButton('text', 'format', '*')" class="p-0.5 hover:text-wa-teal transition-colors" title="Bold"><span class="font-bold text-[10px]">B</span></button>
                                                        <button type="button" @click="insertToButton('text', 'format', '_')" class="p-0.5 hover:text-wa-teal transition-colors" title="Italic"><span class="italic text-[10px]">I</span></button>
                                                        <button type="button" @click="insertToButton('text', 'format', '~')" class="p-0.5 hover:text-wa-teal transition-colors" title="Strike"><span class="line-through text-[10px]">S</span></button>
                                                        <div class="relative inline-block">
                                                            <button type="button" @click="showEmojiPicker = !showEmojiPicker" class="p-0.5 hover:text-wa-teal transition-colors">
                                                            ...
                                                            <div x-show="showEmojiPicker" @click.away="showEmojiPicker = false" x-cloak
                                                                class="absolute bottom-full right-0 mb-2 p-2 bg-white dark:bg-slate-900 rounded-xl shadow-2xl border border-slate-100 dark:border-slate-800 grid grid-cols-5 gap-1.5 z-50 w-40 animate-in fade-in slide-in-from-bottom-1">
                                                                @foreach(['😀', '😃', '😄', '😁', '😅', '😂', '😊', '😍', '🥰', '😘', '😋', '😎', '🤩', '🥳', '👍', '🙌', '🔥', '✨', '✅', '📍'] as $emoji)
                                                                    <button type="button" @click="insertToButton('text', 'emoji', '{{ $emoji }}'); showEmojiPicker = false" class="text-lg hover:scale-125 transition-transform">{{ $emoji }}</button>
                                                                @endforeach
                                                            </div>
                                                        </div>
                                                    </div>
                                                ...
                                                <input type="text" wire:model.live="buttons.{{ $index }}.text" maxlength="25"
                                                    x-ref="textInput_{{ $index }}"
                                                    class="w-full px-4 py-2.5 bg-white dark:bg-slate-950 border-none shadow-sm rounded-xl text-xs font-bold text-slate-900 dark:text-white focus:ring-2 focus:ring-wa-teal/20 transition-all"
                                            ...
                                            @if(($buttons[$index]['type'] ?? '') === 'URL')
                                                <div class="col-span-full">
                                                    <div class="flex items-center justify-between mb-2">
                                                        <label class="block text-[9px] font-black text-slate-400 uppercase">Destination URL</label>
                                                        <button type="button" @click="insertToButton('url', 'var', '@{{1}}')" 
                                                            class="text-[8px] font-black text-wa-teal border border-wa-teal/30 px-2 py-0.5 rounded-lg hover:bg-wa-teal/5 transition-all">
                                                            + Add Variable
                                                        </button>
                                                    </div>
                                                    <input type="url" wire:model.live="buttons.{{ $index }}.url"
                                                        x-ref="urlInput_{{ $index }}"
                                                        class="w-full px-4 py-2.5 bg-white dark:bg-slate-950 border-none shadow-sm rounded-xl text-xs font-bold text-slate-900 dark:text-white focus:ring-2 focus:ring-wa-teal/20 transition-all font-mono"
                                                        placeholder="https://example.com/@{{1}}">
                                                    <p class="text-[8px] text-slate-400 mt-1.5 uppercase font-black tracking-widest italic opacity-60 px-1">One {{1}} variable is allowed at the end of the URL path.</p>
                                                </div>
                                        ...
                                        <div class="p-6 bg-slate-50 dark:bg-slate-900 rounded-3xl border border-wa-teal/20 relative overflow-hidden">
                                            ...
                                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                                                <div>
                                                    ...
                                                    <div class="relative">
                                                        <input type="text" wire:model.live="variableConfig.{{ $var }}.name" readonly
                                                            class="w-full px-4 py-2 bg-slate-100 dark:bg-slate-800/50 border-none rounded-xl text-xs font-bold text-slate-400 dark:text-slate-500 cursor-not-allowed select-none"
                                                            placeholder="{{ $var }}">
                                                        <div class="absolute inset-y-0 right-3 flex items-center">
                                                            <svg class="w-3 h-3 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
                                                        </div>
                                                    </div>
