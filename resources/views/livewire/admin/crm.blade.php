<div class="py-12 bg-slate-50 min-h-screen">
    <div class="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
            <div>
                <h2 class="text-4xl font-black text-slate-900 tracking-tight mb-2">Central <span class="text-wa-teal">CRM</span></h2>
                <p class="text-slate-500 font-bold uppercase tracking-[0.2em] text-[10px]">Super Admin Command Center • v2.1</p>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-12">
            @foreach([
                ['label' => 'Pulse Points', 'value' => $stats['total_users'], 'sub' => '+'.$stats['new_today'].' today', 'color' => 'bg-emerald-500', 'icon' => '⚡'],
                ['label' => 'Active WABA', 'value' => $stats['active_setups'], 'sub' => 'Live Terminals', 'color' => 'bg-indigo-500', 'icon' => '📱'],
                ['label' => 'Offer Pipeline', 'value' => $stats['trial_users'], 'sub' => 'Trial Conversions', 'color' => 'bg-amber-500', 'icon' => '🎁'],
                ['label' => 'Growth Index', 'value' => $stats['new_this_week'], 'sub' => 'L7 Performance', 'color' => 'bg-purple-500', 'icon' => '📈', 'span' => 'lg:col-span-2'],
            ] as $stat)
        ...
        <div class="mb-16">
            <div class="flex items-center justify-between mb-8">
                <div>
                    <h3 class="text-2xl font-black text-slate-900 tracking-tight">Onboarding <span class="text-wa-teal">Friction Funnel</span></h3>
        ...
        <div class="bg-white border border-slate-200 rounded-[3rem] overflow-hidden shadow-sm">
            <div class="p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                <h4 class="text-sm font-black text-slate-900 uppercase tracking-widest">Master <span class="text-slate-400">Registry</span></h4>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="text-slate-400 border-b border-slate-100 bg-slate-50/30">
                            <th class="px-8 py-6 text-[10px] font-black uppercase tracking-[0.2em]">Identity / Workspace</th>
                            ...
                            <th class="px-8 py-6 text-[10px] font-black uppercase tracking-[0.2em]">LTV (Est.)</th>
                            <th class="px-8 py-6 text-[10px] font-black uppercase tracking-[0.2em]">Pulse Health</th>
                            <th class="px-8 py-6 text-right text-[10px] font-black uppercase tracking-[0.2em]">Operations</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100">
                        @foreach($teams as $team)
                        <tr class="group hover:bg-slate-50/50 transition-colors cursor-pointer" wire:click="showUser({{ $team->id }})">
                            <td class="px-8 py-6">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div>
                                        <div class="text-sm font-black text-slate-900 uppercase tracking-tight">{{ $team->name }}</div>
                ...
                <div class="flex h-full">
                    ...
                    <div class="w-80 bg-slate-50 border-r border-slate-100 p-8 flex flex-col justify-between">
                        <div>
                            <div class="flex flex-col items-center text-center mb-8">
                                ...
                                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">{{ $selectedTeam->owner->email }}</p>
                            </div>
                    ...
                    <div class="flex-1 overflow-y-auto p-12 bg-white relative">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-12">
                            {{-- Integration Section --}}
                            <div class="space-y-8">
                                <h5 class="text-sm font-black text-slate-900 uppercase tracking-[0.3em] border-b border-slate-100 pb-4">Integration <span class="text-wa-teal">Telemetrics</span></h5>
                                ...
                                <div class="space-y-6">
                                    ...
                                    <div class="pt-4 mt-4 border-t border-slate-100">
                                        <p class="text-[10px] text-slate-400 font-black uppercase tracking-widest mb-3">Setup Heartbeat</p>
                                        <div class="bg-slate-50 rounded-2xl p-5 border border-slate-100">
                                            @if($selectedTeam->last_webhook_received_at)
                                                <div class="flex items-center justify-between mb-4">
                                                    <span class="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Pulse Live</span>
                                            ...
                                            @else
                                                <div class="flex items-center justify-between">
                                                    <span class="text-[10px] font-black text-slate-300 uppercase tracking-widest">No Pulse Detected</span>
                            ...
                            <div class="space-y-8">
                                ...
                                <div>
                                    <h5 class="text-sm font-black text-slate-900 uppercase tracking-[0.3em] border-b border-slate-100 pb-4 mb-6">Internal <span class="text-amber-500">Admin Notes</span></h5>
                                    <div class="space-y-4">
                                        <textarea wire:model="adminNote" placeholder="Add private notes about this lead/deployment..." class="w-full bg-slate-50 border border-slate-200 rounded-2xl p-5 text-xs text-slate-700 placeholder-slate-400 focus:ring-2 focus:ring-amber-500/10 focus:border-amber-500 transition-all min-h-[120px] shadow-inner"></textarea>
                                        <div class="flex justify-end">
                                            <button wire:click="saveNote" class="px-6 py-3 bg-amber-500 text-white text-[10px] font-black uppercase tracking-widest rounded-xl shadow-lg shadow-amber-500/20 hover:scale-[1.02] active:scale-95 transition-all">
                                                Persist Notes
                                            </button>
                                        </div>
                                    </div>
