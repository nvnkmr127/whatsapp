<div class="space-y-8">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
            <div class="flex items-center gap-3 mb-2">
                ...
                <h1 class="text-3xl font-black text-slate-900 dark:text-white tracking-tight uppercase">Flow <span
                        class="text-wa-green">Intelligence</span></h1>
            </div>
        ...
        <div class="flex flex-col sm:flex-row items-center gap-3">
            ...
            <div class="relative group w-full sm:w-64">
                <input wire:model.live.debounce.300ms="search" type="text"
                    class="w-full pl-10 pr-4 py-3 bg-white dark:bg-slate-900 border-none rounded-xl text-slate-900 dark:text-white placeholder:text-slate-400 focus:ring-2 focus:ring-wa-green/20 transition-all font-medium text-sm shadow-sm"
            ...
            <a href="{{ route('automations.builder') }}"
                class="flex items-center justify-center gap-2 px-8 py-3 bg-slate-900 dark:bg-wa-green text-white dark:text-slate-900 font-black uppercase tracking-widest text-xs rounded-2xl shadow-xl shadow-slate-900/10 dark:shadow-wa-green/20 hover:scale-[1.02] active:scale-95 transition-all w-full sm:w-auto">
            ...
            <table class="w-full text-left">
                <thead>
                    <tr class="border-b border-slate-50 dark:border-slate-800/50">
                        <th class="px-8 py-6 text-[10px] font-black uppercase tracking-widest text-slate-400">Automation
                            Logic</th>
                        <th class="px-8 py-6 text-[10px] font-black uppercase tracking-widest text-slate-400">Activation
                            Trigger</th>
                        ...
                        <tr class="group hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors">
                            ...
                            <td class="px-8 py-6">
                                <div class="flex items-center gap-2">
                                    <span
                                        class="w-2 h-2 rounded-full {{ $bot->is_active ? 'bg-wa-green shadow-lg shadow-wa-green/40' : 'bg-rose-500 shadow-lg shadow-rose-500/40' }}"></span>
                                    <span
                                        class="text-xs font-black uppercase tracking-widest {{ $bot->is_active ? 'text-wa-green' : 'text-rose-500' }}">
    ...
    <x-confirmation-modal wire:model="confirmingDeletion">
        <x-slot name="title">
            <span class="text-slate-900 dark:text-white uppercase font-black tracking-tight">Decommission Flow?</span>
        </x-slot>

        <x-slot name="content">
            <span class="text-slate-500 font-medium tracking-tight">This will permanently erase the automation logic and
                halt all active conversations linked to this flow.</span>
        </x-slot>
