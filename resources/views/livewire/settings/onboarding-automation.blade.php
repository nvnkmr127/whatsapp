    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
            ...
            <div
                ...
                <div class="flex items-baseline gap-2">
                    <span
                        class="text-4xl font-black text-slate-900 dark:text-white">{{ $stats['signups'] > 0 ? round(($stats['completed'] / $stats['signups']) * 100) : 0 }}%</span>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-1 space-y-8">
                    <div
                        ...
                        <p class="text-sm text-slate-500 leading-relaxed mb-6">These templates and delays are used by
                            the <strong>Onboarding Safety Net</strong> and background inactivity monitor.</p>
                                ...
                                @foreach(['whatsapp' => 'WhatsApp Only', 'email' => 'Email Only', 'both' => 'Both (Omnichannel)'] as $value => $label)
                                    <button type="button" wire:click="$set('onboardingChannel', '{{ $value }}')"
                                        ...
                                        <div class="flex items-center gap-3">
                                            ...
                                            <span
                                                class="text-[10px] font-black uppercase tracking-widest {{ $onboardingChannel === $value ? 'text-indigo-600' : 'text-slate-500' }}">{{ $label }}</span>
                                        </div>
                                    </button>
                                @endforeach
                            </div>
                        ...
                        <div
                            ...
                            <h4
                                ...
                                Template Preview</h4>
                            <p class="text-[10px] text-indigo-800/80 dark:text-indigo-400/80 leading-relaxed">Selecting
                                a template will show its content so you can map variables correctly.</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div
                        ...
                        <div class="p-8 md:p-10 space-y-12">
                                ...
                                <div
                                    ...
                                    <div>
                                        ...
                                        <p class="text-[10px] text-slate-400 font-bold uppercase tracking-wider">
                                            Trigger: WhatsApp not connected</p>
                                    </div>
                                    <div class="flex items-center gap-3">
                                        <label
                                        ...
                                        <input type="number" wire:model="firstDelay"
                                            class="w-20 px-3 py-2 bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-xs font-black text-indigo-600 focus:ring-1 focus:ring-indigo-500/20">
                                    </div>
                                </div>
                                <div class="space-y-4">
                                    <div class="relative">
                                        <select wire:model.live="remoteReminderTemplate"
                                            class="w-full px-5 py-4 bg-slate-50 dark:bg-slate-800 border-none rounded-2xl text-slate-900 dark:text-white font-bold cursor-pointer focus:ring-2 focus:ring-indigo-500/20 transition-all appearance-none text-sm">
                                    ...
                                    @if($previews['remote'])
                                        <div
                                            class="p-4 bg-indigo-50/50 dark:bg-indigo-900/10 rounded-2xl border border-indigo-100/50 dark:border-indigo-900/20">
                                            <p
                                                class="text-[10px] text-indigo-600 dark:text-indigo-400 font-bold uppercase tracking-widest mb-2">
                                                Message Preview</p>
                                            <p
                                                class="text-xs text-slate-600 dark:text-slate-400 whitespace-pre-wrap font-medium">
                                                {{ $previews['remote'] }}</p>
                                        </div>
                                    @endif
                                    ...
                                    <div class="flex flex-col gap-2">
                                        <label
                                            class="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Map
                                            Positional Variables</label>
                                        <div class="flex flex-wrap gap-2 mb-2">
                                            @foreach($availablePlaceholders as $key => $label)
                                                <button type="button"
                                                    wire:click="addPlaceholder('remoteReminderVariables', '{{ $key }}')"
                                                    title="{{ $label }}"
                            ...
                            <div class="space-y-6">
                                ...
                                <div class="space-y-4">
                                    <div class="relative">
                                        <select wire:model.live="profileReminderTemplate"
                                            class="w-full px-5 py-4 bg-slate-50 dark:bg-slate-800 border-none rounded-2xl text-slate-900 dark:text-white font-bold cursor-pointer focus:ring-2 focus:ring-indigo-500/20 transition-all appearance-none text-sm">
                                            <option value="">Choose a template...</option>
                                            @foreach($templates as $template)
                                                <option value="{{ $template['name'] }}">{{ $template['name'] }}</option>
                                    ...
                                    <div class="flex flex-col gap-2">
                                        ...
                                        <div class="flex flex-wrap gap-2 mb-2">
                                            @foreach($availablePlaceholders as $key => $label)
                                                <button type="button"
                                                    wire:click="addPlaceholder('profileReminderVariables', '{{ $key }}')"
                                                    title="{{ $label }}"
                                                    class="px-3 py-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-[10px] font-black text-indigo-600 hover:bg-indigo-600 hover:text-white transition-all">
                                                    {{ $key }}
                                                </button>
                            ...
                            <div class="space-y-6">
                                <div
                                    ...
                                    <div>
                                        ...
                                        <p class="text-[10px] text-slate-400 font-bold uppercase tracking-wider">
                                            Trigger: No campaigns created</p>
                                    </div>
                                    <div class="flex items-center gap-3">
                                        <label
                                            class="text-[10px] font-black uppercase tracking-widest text-slate-400">Delay
                                ...
                                <div class="space-y-4">
                                    <div class="relative">
                                        <select wire:model.live="campaignTutorialTemplate"
                                            class="w-full px-5 py-4 bg-slate-50 dark:bg-slate-800 border-none rounded-2xl text-slate-900 dark:text-white font-bold cursor-pointer focus:ring-2 focus:ring-indigo-500/20 transition-all appearance-none text-sm">
                                            <option value="">Choose a template...</option>
                                            @foreach($templates as $template)
                                                <option value="{{ $template['name'] }}">{{ $template['name'] }}</option>
                                    ...
                                    <div class="flex flex-col gap-2">
                                        ...
                                        <div class="flex flex-wrap gap-2 mb-2">
                                            @foreach($availablePlaceholders as $key => $label)
                                                <button type="button"
                                                    wire:click="addPlaceholder('campaignTutorialVariables', '{{ $key }}')"
                                                    title="{{ $label }}"
                                                    class="px-3 py-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-[10px] font-black text-indigo-600 hover:bg-indigo-600 hover:text-white transition-all">
                                                    {{ $key }}
                                                </button>
